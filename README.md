...
| Python 订单模块 | 已实现并测试 | 支持下单、订单列表、核销、6 小时倒计时、钱包充值/提现和演示托管结算 |
...
- 管理员端：商家审核、风险日志和数据看板。
- 订单后端：原子扣减库存、生成唯一 6 位数字取货码、6 小时领取倒计时、自动完成、0.1% 服务费、钱包入账、月销售与销量统计、商家数据隔离和防止重复核销。
...
订单状态：`0` 待领取、`1` 已完成、`2` 已关闭。订单创建后有 6 小时领取时间，商家可以通过订单 ID 或取货码手动核销；超时后系统自动完成订单。两种完成方式都会按订单总额扣除 0.1% 演示服务费，并把剩余金额标记为已结算给商家。

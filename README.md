...
| Python 订单模块 | 已实现并测试 | 支持下单、订单列表、关门时间倒计时、核销、钱包充值/提现和演示托管结算 |
...
- 学生端：登录注册、个性化推荐、猜你喜欢、商品详情、收藏、模拟微信支付、钱包概览、订单查询和取货凭证。
- 商家端：简化发布商品、食品/饮品盲盒、钱包与月度经营概览、订单管理、取货码核销和结算明细。
- 管理员端：商家审核、风险日志和数据看板。
- 订单后端：原子扣减库存、生成唯一 6 位数字取货码、按商家关门时间计算领取倒计时、自动完成、0.1% 服务费、钱包入账、月销售与销量统计、商家数据隔离和防止重复核销。
...
| `GET` | `/api/orders/summary` | 学生/商家 | 查询钱包余额、托管金额与角色对应的月度数据 |
| `POST` | `/api/orders/wallet/recharge` | 学生/商家 | 演示钱包充值，金额使用两位小数字符串 |
| `POST` | `/api/orders/wallet/withdraw` | 学生/商家 | 从可用余额演示提现，托管金额不可提现 |
...
订单状态：`0` 待领取、`1` 已完成、`2` 已关闭。商家发布商品时填写每日开门与关门时间，订单领取截止为下单后的下一次关门时间；若商品更早到期，则以商品有效期为准。商家可以通过订单 ID 或取货码手动核销，到时后系统自动完成订单。两种完成方式都会按订单总额扣除 0.1% 演示服务费，并把剩余金额标记为已结算给商家。

订单金额在数据库中使用 `DECIMAL(10,2)`，接口以两位小数字符串返回，例如 `"12.00"`，避免浮点精度问题。商家只有在手动核销或订单超时自动完成后才会收到净额，且同一订单不会重复入账；“本月销量”按商品份数统计，“本月订单”按订单笔数统计。钱包充值与提现需要 6 位演示支付密码，提现只扣除可用余额，不会动用托管中的订单款。当前微信支付、平台托管、钱包和商家到账均为比赛演示数据，不会发生真实扣款或转账，模拟微信支付也不会扣减食愿钱包余额。

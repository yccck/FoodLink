...
| Python 订单模块 | 已实现并测试 | 支持下单、订单列表、领取倒计时、核销、5 分钟限时退款、食品问题审核退款和 0.1% 服务费结算 |
...
- 商家端：简化发布商品、食品/饮品盲盒、月度经营概览、订单管理、取货码核销和结算明细。
- 管理员端：商家审核、风险日志和数据看板。
...
订单金额在数据库中使用 `DECIMAL(10,2)`，接口以两位小数字符串返回，例如 `"8.80"`，避免浮点精度问题。学生仅能在付款后 5 分钟内且尚未核销时自行取消，成功后恢复库存并标记为原路退款。超过 5 分钟不能自行退款；只有商家实际核销后发现食品质量问题，学生才可提交问题说明和可选照片，由管理员审核。审核通过后原路退款并冲回该笔商家结算，驳回则保持原订单结算。关门自动完成的未领取订单和食品领取期限已过的订单不能以食品问题申请退款。当前微信支付和商家到账均为比赛演示流程，不会发生真实扣款或转账。

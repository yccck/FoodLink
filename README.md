...
| Python 订单模块 | 已实现并测试 | 支持下单、订单列表、核销、6 小时倒计时、钱包概览和演示托管结算 |
...
订单金额在数据库中使用 `DECIMAL(10,2)`，接口以两位小数字符串返回，例如 `"12.00"`，避免浮点精度问题。商家只有在手动核销或订单超时自动完成后才会收到净额，且同一订单不会重复入账；“本月销量”按商品份数统计，“本月订单”按订单笔数统计。当前微信支付、平台托管、钱包和商家到账均为比赛演示数据，不会发生真实扣款或转账，模拟微信支付也不会扣减食愿钱包余额。
